...
while True:
	for ratio in ratios:
		garnet = garnets.Garnets(nb_states, nb_actions, nb_next_state_transition, env_type, self_transitions=self_transitions)
		...
		for nb_trajectories in nb_trajectories_list:
			...
			print("perf RMDP_based_alorithm:\t" + str(perf_RMDP_based_alorithm))
			...
			for epsilon in epsilons:
				...
				perf_soft_SPIBB_simplex_1step = spibb.policy_evaluation_exact(soft_SPIBB_simplex_1step.pi, r_reshaped, current_proba, gamma)[0][0]
				print("perf Soft-SPIBB with simplex:\t" + str(perf_soft_SPIBB_simplex_1step))

				# Computes the Soft-SPIBB-max-Q policy
				soft_SPIBB_max_Q_1step = spibb.spibb(
					gamma, nb_states, nb_actions, pi_b, mask, model.transitions, reward_model, 'Soft_SPIBB_max_Q',
					errors=errors, epsilon=2*epsilon, max_nb_it=1
				)
				soft_SPIBB_max_Q_1step.fit()
				# Evaluates the Soft-SPIBB-max-Q performance
				perf_soft_SPIBB_max_Q_1step = spibb.policy_evaluation_exact(soft_SPIBB_max_Q_1step.pi, r_reshaped, current_proba, gamma)[0][0]
				print("perf Soft-SPIBB-max-Q:\t\t" + str(perf_soft_SPIBB_max_Q_1step))
				...
				perf_soft_SPIBB_sort_Q_1step = spibb.policy_evaluation_exact(soft_SPIBB_sort_Q_1step.pi, r_reshaped, current_proba, gamma)[0][0]
				print("perf Soft-SPIBB-sort-Q:\t\t" + str(perf_soft_SPIBB_sort_Q_1step))
				...
				# Evaluates the Soft-SPIBB-simplex performance
				perf_soft_SPIBB_simplex = spibb.policy_evaluation_exact(soft_SPIBB_simplex.pi, r_reshaped, current_proba, gamma)[0][0]
				print("perf Soft-SPIBB with simplex:\t" + str(perf_soft_SPIBB_simplex))

				# Computes the Soft-SPIBB-max-Q policy
				soft_SPIBB_max_Q = spibb.spibb(
					gamma, nb_states, nb_actions, pi_b, mask, model.transitions, reward_model, 'Soft_SPIBB_max_Q',
					errors=errors, epsilon=2*epsilon
				)
				soft_SPIBB_max_Q.fit()
				# Evaluates the Soft-SPIBB-max-Q performance
				perf_soft_SPIBB_max_Q = spibb.policy_evaluation_exact(soft_SPIBB_max_Q.pi, r_reshaped, current_proba, gamma)[0][0]
				print("perf Soft-SPIBB-max-Q:\t\t" + str(perf_soft_SPIBB_max_Q))
				...
				# Evaluates the Soft-SPIBB-sort-Q performance
				perf_soft_SPIBB_sort_Q = spibb.policy_evaluation_exact(soft_SPIBB_sort_Q.pi, r_reshaped, current_proba, gamma)[0][0]
				print("perf Soft-SPIBB-sort-Q:\t\t" + str(perf_soft_SPIBB_sort_Q))
				...
				results.append([seed, gamma, nb_states, nb_actions, 4,
					...
					perf_soft_SPIBB_simplex, perf_soft_SPIBB_max_Q, perf_soft_SPIBB_sort_Q,
					perf_soft_SPIBB_simplex_1step, perf_soft_SPIBB_max_Q_1step, perf_soft_SPIBB_sort_Q_1step,
	...
	df = pd.DataFrame(results, columns=['seed','gamma','nb_states','nb_actions','nb_next_state_transition',
		...
		'perf_RaMDP', 'perf_RMDP_based_alorithm', 'perfhcope_doubly_robust',
